 ...
 * @version 1.0.1
 * @description Servidor MCP para consulta de datos catastrales publicos de Uruguay
 */
...
function createCatastroServer() {
    const server = new McpServer({ name: "uy-catastro-mcp", version: "1.0.1" });
...
const app = createServer(async (req, res) => {
    ...
    if (req.url === "/health" || req.url === "/") {
          ...
          res.end(JSON.stringify({ status: "ok", server: "uy-catastro-mcp", version: "1.0.1" }));
          return;
    }
...
app.listen(PORT, () => {
    console.log("Uy Catastro MCP server v1.0.1 listening on http://localhost:" + PORT + MCP_PATH);
});
